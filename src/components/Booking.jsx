...
import { Send, Clock, User, Phone, Mail, X, CreditCard, CheckCircle2, Plus, Trash2, Calendar, Loader2 } from 'lucide-react';
...
const Booking = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    message: '',
    sessions: [{ date: '', hours: '2' }] // Array of dynamic sessions
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // Hardcoding the key to instantly bypass Vite's browser-side caching of the old env variable
    const apiKey = "e63b90a6-896e-47d7-a926-e5987e56e515";
    ...
    try {
      ...
      if (result.success || !import.meta.env.VITE_WEB3FORMS_ACCESS_KEY) {
        // We succeed if it actually succeeded, or if we don't have a real key yet (for demo purposes)
        confetti({
          particleCount: 200,
          spread: 120,
          origin: { y: 0.4 },
        ...
        alert(`Booking Error: ${result.message || "Please check your Web3Forms Access Key."}`);
      }
    } catch (error) {
  ...
  const addSession = () => {
    setFormData({ 
      ...formData, 
      sessions: [...formData.sessions, { date: '', hours: '2' }] 
    });
  };
  ...
  // Calculate Total
  const estimatedHours = formData.sessions.reduce((sum, session) => {
    return sum + (parseInt(session.hours) || 0);
  }, 0);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className={`relative w-full bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col transition-all duration-300 ${
              isSubmitted ? 'max-w-md' : 'max-w-4xl'
            }`}
          >
            {/* Header */}
            {!isSubmitted && (
              ...
              {!isSubmitted ? (
                  ...
                  <form id="booking-form" onSubmit={handleSubmit} className="space-y-4 md:space-y-5 flex-grow">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input 
                            ...
                            className="w-full pl-9 pr-3 py-2.5 md:py-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm" placeholder="John Doe"
                          />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input 
                            ...
                            className="w-full pl-9 pr-3 py-2.5 md:py-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm" placeholder="+221 78 158 27 40"
                          />
                        </div>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input 
                            ...
                            className="w-full pl-9 pr-3 py-2.5 md:py-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm" placeholder="john@example.com"
                          />
                        </div>
                      </div>
                      <div>
                        ...
                        <select 
                          ...
                          className="w-full px-3 py-2.5 md:py-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm font-medium"
                        >
                    ...
                    <div>
                        ...
                        <AnimatePresence>
                          {formData.sessions.map((session, index) => (
                            <motion.div 
                              ...
                              className="flex items-start sm:items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200 flex-col sm:flex-row"
                            >
                              {/* Date Picker */}
                              <div className="relative flex-grow w-full sm:w-auto">
                                ...
                                <input 
                                  ...
                                  className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm shadow-sm" 
                                />
                              </div>
                              ...
                              <div className="flex items-center gap-3 w-full sm:w-auto">
                                ...
                                <div className="relative flex-grow">
                                  ...
                                  <select 
                                    value={session.hours} 
                                    onChange={(e) => handleSessionChange(index, 'hours', e.target.value)}
                                    className="w-full pl-9 pr-8 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm font-medium shadow-sm appearance-none"
                                  >
                                ...
                                {formData.sessions.length > 1 && (
                                  <button 
                                    ...
                                    className="w-9 h-9 shrink-0 flex items-center justify-center bg-red-50 text-red-500 hover:bg-red-100 rounded-lg transition-colors"
                                  >
                                    <Trash2 size={16} />
                                  </button>
                                )}
                      ...
                      <button 
                        type="button"
                        onClick={addSession}
                        className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-slate-300 text-slate-500 hover:border-primary hover:text-primary hover:bg-primary/5 rounded-xl text-sm font-bold transition-all"
                      >
                        <Plus size={16} />
                        Add Another Session
                      </button>
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        className="w-full px-3 py-2.5 md:py-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/50 text-sm resize-none"
                        placeholder="Tell us about your driving experience or specific goals..."
                      ></textarea>
                    </div>
                  </form>
                  ...
                  <div className="lg:w-80 shrink-0">
                    <div className="bg-slate-900 rounded-2xl p-6 text-white sticky top-0 shadow-xl border border-slate-800">
                      ...
                      >
                        ...
                        {isSubmitting ? 'Sending Request...' : 'Confirm & Book'}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center text-center py-6">
                  ...
                  <p className="text-sm text-slate-600 mb-8 leading-relaxed max-w-sm">
                    ...
                    Please check your inbox and click the link to confirm your driving lessons.
                  </p>
